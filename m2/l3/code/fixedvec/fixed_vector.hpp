...
void serialize(std::ostream & os, fixed_vector<T> const & v) {
  if (v.size_ <= 0) { return; }
  os << v.buffer_[0];
  for (int i = 1; i < v.size_; ++i) { os << " , " << v.buffer_[gsl::narrow<std::size_t>(i)]; }
}
...
class fixed_vector<T *> {
  public:
    ...
    friend std::ostream & operator<< <T *>(std::ostream & os, fixed_vector const & v);
...
fixed_vector<std::string>::fixed_vector(fixed_vector const & other)
  : capacity_{other.capacity_}, size_{other.size_},
...
fixed_vector<std::string> &
    fixed_vector<std::string>::operator=(fixed_vector const & other) {
  if (this == &other) { return *this; }

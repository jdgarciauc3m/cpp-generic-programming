...
class allocated_storage {
  public:
    explicit allocated_storage(int n)
      : capacity_{n}, buffer_{std::make_unique<T[]>(gsl::narrow<std::size_t>(n))} {
      Expects(n > 0);
    }
...
std::ostream & operator<<(std::ostream & os, fixed_vector<T, S> const & v) {
  ...
  os << v.storage_[0];
  for (int i = 1; i < v.size_; ++i) { os << " , " << v.storage_[i]; }
  return os;
}
...
class fixed_vector<T, local_storage<T>> {
  public:
    explicit fixed_vector(int n) { Expects(n >= 0 and n < storage_.capacity()); }

...
\end{columns}
  ...
  \item Two operators have a default initial value: \cppkey{\&\&} (\cppkey{true}) and \cppkey{||} (\cppkey{false}).
\begin{columns}[T]
...
\begin{itemize}
  ...
  \mode<presentation>{\vfill\pause}
  \item \textbad{Note}: Some operations might make sens only for one folding.
